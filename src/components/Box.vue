  ...
  <div v-show="displaySet" :class="'product product--' + set.type + ' product--' + selectedFormatLegality">
    ...
    <a :href="'https://netrunnerdb.com/en/set/' + set.nrdb" target="_blank" class="product__box">
      ...
      <div class="product__info">
        ...
        <span v-if="set.mwlBanned" class="product__bans">Banned Cards: <span v-for="ban in set.mwlBanned">{{ ban }}<span class="product__bans-connect">, </span></span></span>
        <span v-if="set.mwlRestricted" class="product__bans">Restricted Cards: <span v-for="restrict in set.mwlRestricted">{{ restrict }}<span class="product__bans-connect">, </span></span></span>
      </div>
    </a>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    hasMwl() {
      return (this.set.mwlBanned || this.set.mwlRestricted);
    },
    mwlCount() {
      let count = 0;

      if (this.set.mwlBanned) {
        count += this.set.mwlBanned.length;
      }

      if (this.set.mwlRestricted) {
        count += this.set.mwlRestricted.length;
      }
